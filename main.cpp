...
int main(){
    ...
    while(std::cin >> x){
    // I don't really understand how this while loop works, and though other people use this it seems to behave strangely for me. I think it might be the problem since entering things other than integers (which I am generally required to do in order to get anything to happen) will probably effect the program's ability to determine if an index is greater than another index.
        ...
        // When I type a letter after the numbers, the program doesn't stop and I get that same error that shows a 'breakpoint,' this time down at the for loop. When I type a letter before the numbers, the program finishes and tells me that the vector is empty.
        
        // When I type in a really long number (something like 22 characters, and then just hit enter) the program finishes, and says that the vector is empty. When I type two long numbers, it does the same. This is the only way that I can get the program to respond with only numbers in the stream.
